...
async function execute(thread) {
    ...
    if (thread.parentId != process.env.POLL_CHANNEL_ID) {
        ...
        const defaultRole = thread.guild.roles.cache.get(process.env.DEFAULT_ROLE_ID);
        const parentChannel = thread.parent;
        
        if (parentChannel && 
            parentChannel.type === ChannelType.GuildText &&
            parentChannel.permissionsFor(defaultRole)?.has(PermissionsBitField.Flags.ViewChannel)) {
