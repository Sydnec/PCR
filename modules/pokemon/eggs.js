...
//
// Le bébé a les chances d'être shiny d'une apparition sauvage, multipliées par
// `shinyParentMultiplier` pour chaque parent shiny : ×2 pour un, ×4 pour deux.
...
// vrai en SQL. Rend { id, is_shiny }, ou null.
function claimParent(userId, { speciesId, isShiny, sex, pokemonId = null }, cb) {
  db.get(
    `UPDATE pokemon_owned SET sterile = 1
      ...
      RETURNING id, is_shiny`,
    [userId, speciesId, isShiny ? 1 : 0, sex ?? null, pokemonId ?? null, pokemonId ?? null],
    (err, row) => cb(err, row ?? null)
  );
}

// Combien de fois les chances de shiny d'une apparition sont multipliées pour
// un œuf, selon le nombre de ses parents shiny. Le même calcul sert au tirage
// et à l'affichage.
...
// ordonné avec compensation, comme une évolution : le père, puis la mère, puis
...
export function layEgg(userId, firstGroup, secondGroup, cb) {
  ...
  getIncubatingEgg(userId, (err, current) => {
    ...
    claimParent(userId, father, (err, fatherRow) => {
      if (err) return cb(err);
      if (!fatherRow) return noFertile(fatherSpecies, father);
      const fatherId = fatherRow.id;

      claimParent(userId, mother, (err, motherRow) => {
        if (err || !motherRow) {
        ...
        // Retenu à la ponte : les parents peuvent partir avant l'éclosion.
        const shinyParents = (fatherRow.is_shiny ? 1 : 0) + (motherRow.is_shiny ? 1 : 0);
        ...
        db.run(
          `INSERT INTO pokemon_eggs
             (user_id, species_id, father_id, mother_id, father_species_id, mother_species_id,
              shiny_parents, status, messages, hatch_messages, laid_at, hatch_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, 'INCUBATING', 0, ?, ?, ?)`,
          [
            userId,
            plan.baby.id,
