...
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    ...
    db.run('CREATE TABLE IF NOT EXISTS messages (id TEXT PRIMARY KEY, link TEXT, expire_at INTEGER)', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table messages :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS voice_time (user_id TEXT PRIMARY KEY, total_time_ms INTEGER DEFAULT 0, longest_session INTEGER DEFAULT 0, join_time INTEGER)', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table voice_time :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS message_stats (user_id TEXT, channel_id TEXT, date TEXT, count INTEGER DEFAULT 0, PRIMARY KEY(user_id, channel_id, date))', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table message_stats :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS emoji_stats (user_id TEXT, emoji TEXT, count INTEGER DEFAULT 0, PRIMARY KEY(user_id, emoji))', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table emoji_stats :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS reaction_stats (user_id TEXT, count INTEGER DEFAULT 0, PRIMARY KEY(user_id))', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table reaction_stats :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS command_stats (command TEXT PRIMARY KEY, count INTEGER DEFAULT 0)', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table command_stats :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS message_reactions (message_id TEXT PRIMARY KEY, count INTEGER DEFAULT 0)', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table message_reactions :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS word_stats (user_id TEXT, word TEXT, count INTEGER DEFAULT 0, PRIMARY KEY(user_id, word))', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table word_stats :', err);
      }
    });
    ...
    db.run('CREATE TABLE IF NOT EXISTS progress (id TEXT PRIMARY KEY, last_processed_timestamp INTEGER)', (err) => {
      if (err) {
        handleException('Erreur lors de la création de la table progress :', err);
      }
    });
  }
});

// // Fonction de "remontée dans le temps" pour remplir la BDD avec les événements passés 
// // entre le 01/01/2025 00:00:00 et le 29/08/2025 à 00:28
...
// const LIMITE_TIMESTAMP_MAX = new Date('2025-08-30T13:07:00Z').getTime();
// const LIMITE_TIMESTAMP_MIN = new Date('2024-12-31T23:00:00Z').getTime();

// /**
//  * Remplit la BDD avec les événements passés entre LIMITE_TIMESTAMP_MIN et LIMITE_TIMESTAMP_MAX.
...
//     if (err) return handleException('Erreur lecture progression remontée :', err);
//     let since = row ? row.last_processed_timestamp : LIMITE_TIMESTAMP_MIN;
//     if (since >= LIMITE_TIMESTAMP_MAX) return; // Déjà fait

//     log(`Début de la remontée dans le temps pour la période : ${new Date(LIMITE_TIMESTAMP_MIN).toISOString()} - ${new Date(LIMITE_TIMESTAMP_MAX).toISOString()}`);

//     // Pour chaque channel textuel, on remonte les messages dans la période spécifiée
...
//               // Ignore les messages après la limite max
//               if (msg.createdTimestamp >= LIMITE_TIMESTAMP_MAX) continue;
              
//               // Arrête si on atteint des messages avant la limite min
//               if (msg.createdTimestamp < LIMITE_TIMESTAMP_MIN) {
//                 done = true;
//                 break;
//               }
...
//                 if (err) handleException('Erreur insertion message_stats rétro :', err);
//               });
...
//                     if (err) handleException('Erreur insertion emoji_stats rétro :', err);
//                   });
//                 }
...
//                     if (err) handleException('Erreur insertion word_stats rétro :', err);
//                   });
//                 }
//               }
...
//                     if (err) handleException('Erreur insertion command_stats rétro :', err);
//                   });
//                   break;
//                 }
//               }
...
//                         if (err) handleException('Erreur insertion reaction_stats rétro :', err);
//                       });
//                     }
//                   } catch (e) {
...
//                   if (err) handleException('Erreur insertion message_reactions rétro :', err);
//                 });
//               }

//               // Mise à jour de la progression avec le timestamp du message traité
//               db.run('INSERT OR REPLACE INTO progress (id, last_processed_timestamp) VALUES (?, ?)', [REMONTÉE_ID, msg.createdTimestamp]);
//               beforeId = msg.id;
//             }
//             if (messages.size < 100) break;
//           } catch (e) {
//             handleException('Erreur lors de la remontée dans le temps sur', channel.id, e);
//             break;
//           }
//         }
//       }
...
//     db.run('INSERT OR REPLACE INTO progress (id, last_processed_timestamp) VALUES (?, ?)', [REMONTÉE_ID, LIMITE_TIMESTAMP_MAX]);
//     log('Remontée dans le temps terminée pour la période 2025.');
//   });
// }
